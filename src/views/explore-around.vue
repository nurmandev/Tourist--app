<template>
  ...
  >
    ...
    <div v-for="(mode, index) in data.transportModes" :key="mode.title" class="mb-6">
      ...
      <div class="bg-white px-3 pt-4 pb-6 rounded-b-lg">
        ...
        <p v-if="mode?.time" class="flex bg-white items-center gap-x-2 mt-4 text-sm">
          <AlarmClock :size="19" /> {{ mode?.time }}
        </p>
        <!-- Render "A Trip" link for indices other than the first -->
        <p v-if="index !== 0" class="mt-4 text-right">
          <a
